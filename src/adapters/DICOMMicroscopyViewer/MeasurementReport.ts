...
interface MicroscopyToolClass {
  ...
  getMeasurementData(
    measurementContent: MeasurementContentItem[]
  ): number[][] | number[][][];
...
function getMeasurementGroup(
  ...
  return new TID1501MeasurementGroup(
    Measurements as unknown as ConstructorParameters<
      typeof TID1501MeasurementGroup
    >[0]
  );
}
...
export default class MeasurementReport {
  ...
  static generateReport(
    ...
    const MeasurementReportInstance = new TID1500MeasurementReport({
      TID1501MeasurementGroups:
        allMeasurementGroups as unknown as ConstructorParameters<
    ...
    const contentItem = MeasurementReportInstance.contentItem(
      derivationSourceDatasetWithMeta as unknown as Parameters<
        typeof MeasurementReportInstance.contentItem
      >[0],
      options
    );
  ...
  static generateToolState(
    dataset: ToolStateDataset
  ): ToolStateMeasurementData {
    ...
    const imagingMeasurementContent = contentSequenceArray.find(
      (item): item is ContentSequenceItem =>
        item.ConceptNameCodeSequence?.CodeMeaning === REPORT
    );
    ...
    const measurementGroups = toArray(
      ...
      (item): item is ContentSequenceItem =>
        item.ConceptNameCodeSequence?.CodeMeaning === GROUP
    );
      ...
      Object.keys(
        ...
        const measurementContent = measurementGroupContentSequence.filter(
          (item): item is ContentSequenceItem & MeasurementContentItem =>
            item.GraphicType === measurementType.toUpperCase() &&
            item.GraphicData !== undefined
        );
  ...
  static registerTool(toolClass: {
    ...
    TID300Representation: new (args: any) => TID300Instance;
    getMeasurementData(
      measurementContent: MeasurementContentItem[]
    ): number[][] | number[][][];
    getTID300RepresentationArguments(
    ...
    MeasurementReport.MEASUREMENT_BY_TOOLTYPE[tc.graphicType] =
      tc.utilityToolType;
  }
}
