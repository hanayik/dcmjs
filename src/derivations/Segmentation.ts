...
export default class Segmentation extends DerivedPixels {
  ...
  private get _referencedDatasets(): SegmentationReferencedDataset[] {
    return this
      .referencedDatasets as unknown as SegmentationReferencedDataset[];
  }

  constructor(
    datasets: NaturalizedDataset[],
    options: SegmentationOptions = { includeSliceSpacing: true }
  ) {
  ...
  derive(): void {
      ...
      for (let i = 0; i < this._referencedDatasets.length; i++) {
        ReferencedInstanceSequence.push({
          ...
          ReferencedSOPInstanceUID:
            this._referencedDatasets[i].SOPInstanceUID || "",
        });
      }
    ...
    if (!this.options.includeSliceSpacing) {
      ...
      if (this.dataset.SharedFunctionalGroupsSequence?.PixelMeasuresSequence) {
        delete this.dataset.SharedFunctionalGroupsSequence.PixelMeasuresSequence
  ...
  private _addSegmentPixelDataFromLabelmaps(
    labelmaps: Uint8Array[],
    segmentIndex: number
  ): void {
    const dataset = this.dataset;
  ...
  private _addPerFrameFunctionalGroups(
    ReferencedSegmentNumber: string,
    referencedFrameNumbers: number[]
  ): void {
    ...
    for (let i = 0; i < referencedFrameNumbers.length; i++) {
      ...
      const referencedPerFrame =
        this._referencedDataset.PerFrameFunctionalGroupsSequence;
      if (referencedPerFrame && referencedPerFrame[frameNumber - 1]) {
        ...
        if (
          !this.dataset.SharedFunctionalGroupsSequence?.PlaneOrientationSequence
        ) {
      ...
      PerFrameFunctionalGroupsSequence.push(
        perFrameFunctionalGroups as PerFrameFunctionalGroup
      );
    }
  }
