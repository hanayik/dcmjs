...
import { ValueRepresentation, type VRType, type TransferSyntax, type DicomDataset } from "./ValueRepresentation";
...
type IsBulkdataFn = (this: DictCreator, header: TagHeader, options: DictCreatorOptions) => boolean | undefined;
...
interface PixelUndefinedParseLevelState extends BaseParseLevelState {
    type: "PixelUndefined";
    dict: DicomDict;
    values: (ArrayBuffer | Uint8Array | ArrayBufferLike | (ArrayBuffer | Uint8Array | ArrayBufferLike)[])[];
...
export class DictCreator {
    ...
    handleItem(
        ...
        if (parentWithHandleItem.handleItem) {
            ...
            return parentWithHandleItem.handleItem.call(this, header, stream, tsuid, options) as boolean;
        }
    ...
    handleSequenceDelimitationEnd(
        ...
        const currentSeq = this.current as SequenceParseLevelState | PixelUndefinedParseLevelState;
    ...
    handlePixelItem(
        ...
        pixelCurrent.values.push(bytes as ArrayBuffer | Uint8Array | ArrayBufferLike);
        if (pixelCurrent.offsets?.length) {
            const { nextFrameIndex } = pixelCurrent;
            const nextOffset = pixelCurrent.offsets[nextFrameIndex!] ?? Number.MAX_VALUE;
            const pixelOffset = stream.offset - pixelCurrent.offsetStart!;
            if (pixelOffset <= nextOffset && pixelCurrent.values.length > nextFrameIndex!) {
                ...
                pixelCurrent.values[frameIndex].push(
                    pixelCurrent.values.pop() as ArrayBuffer | Uint8Array | ArrayBufferLike
                );
            }
    ...
    handlePixelDefined(
        ...
        const numberOfFrames = (typeof numberOfFramesValue === "number" ? numberOfFramesValue : 1) || 1;
        ...
        const bitsAllocated = typeof bitsAllocatedValue === "number" ? bitsAllocatedValue : 0;
     ...
     */
    isBulkdata(header: TagHeader, options: DictCreatorOptions): boolean | undefined {
