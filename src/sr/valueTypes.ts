...
type TemporalRangeType = (typeof TemporalRangeTypes)[keyof typeof TemporalRangeTypes];
...
type PixelOriginInterpretation = (typeof PixelOriginInterpretations)[keyof typeof PixelOriginInterpretations];
    ...
    constructor(options: ContentItemOptions) {
        ...
        if (options.relationshipType !== undefined) {
            if (!(Object.values(RelationshipTypes).indexOf(options.relationshipType) !== -1)) {
                throw new Error(`Invalid relationship type ${options.relationshipType}`);
            }
...
class ScoordContentItem extends ContentItem {
    ...
    constructor(options: ScoordContentItemOptions) {
        ...
        if (Array.isArray(graphicData[0])) {
            graphicData = ([] as number[]).concat.apply([], graphicData as number[][]);
        }
        this.GraphicData = graphicData as number[];
        const pixelOriginInterpretation = options.pixelOriginInterpretation || PixelOriginInterpretations.VOLUME;
        if (typeof pixelOriginInterpretation !== "string") {
            throw new Error("Option 'pixelOriginInterpretation' must have type String.");
        }
        if (Object.values(PixelOriginInterpretations).indexOf(pixelOriginInterpretation) === -1) {
            throw new Error(`Invalid pixel origin interpretation '${pixelOriginInterpretation}'.`);
        }
