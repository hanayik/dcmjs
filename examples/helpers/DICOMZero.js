class DICOMZero {
            ...
            if (this.fileIndex === this.dataTransfer.files.length) {
                ...
                if (this.multiframe.SOPClassUID == dcmjs.data.DicomMetaDictionary.sopClassUIDsByName["Segmentation"]) {
                    ...
                    try {
                        ...
                        statusCallback(
                            `Created ${this.multiframe.NumberOfFrames} frame multiframe object and segmentation.`
                        );
                    } catch (e) {
                    ...
                    this.multiframe.SOPClassUID ==
                    dcmjs.data.DicomMetaDictionary.sopClassUIDsByName["ParametricMapStorage"]
                ) {
                    statusCallback(`Creating parametric map...`);
                    try {
                        this.pm = new dcmjs.derivations.ParametricMap([this.multiframe]);
                        statusCallback(
                            `Created ${this.multiframe.NumberOfFrames} frame multiframe object and parametric map.`
                        );
                    } catch (e) {
    ...
    extractFromZipArrayBuffer(arrayBuffer, finishCallback = function () {}) {
        ...
        JSZip.loadAsync(arrayBuffer).then(this.handleZip.bind(this));
    }
